...
export class MergeLayersProgram {
    ...
    void main()
    {
        gl_FragColor = vec4(texture2D(u_colorTexture, v_textureCoordinate).rgb, texture2D(u_alphaTexture, v_textureCoordinate).r);
    }`;
    ...
    public merge(renderTarget: RenderTarget | null,
        param: {
            targetRect: Rect,
            alphaSource: Texture,
            colorSource: Texture,
        ...
        this.renderObject.uniforms.push({name: "u_alphaTexture", value: param.alphaSource});
        this.renderObject.uniforms.push({name: "u_colorTexture", value: param.colorSource});
